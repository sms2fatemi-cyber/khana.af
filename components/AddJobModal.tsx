  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        description: formData.description, phone_number: formData.phoneNumber,
        location: formData.location, images: allImages, owner_id: userPhone, status: 'PENDING'
      };
      const result = await supabase.from(TABLES.JOBS).insert([payload]);
          ...
          <div className="absolute inset-0 z-[110] bg-white flex flex-col animate-in fade-in duration-300">
            ...
            <div className="flex-1 relative">
              ...
              <div className="absolute bottom-10 left-8 right-8 z-[1000]">
                <button onClick={() => setView('form')} disabled={isMapMoving} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black shadow-xl active:scale-95 transition-all">تایید موقعیت</button>
              </div>
            </div>
          </div>
        )}
        <div className="p-5 border-b flex justify-between items-center bg-white shrink-0">
          <h2 className="font-black text-xl text-gray-800">ثبت شغل</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div className="relative flex-1">
                    <input type="text" value={formData.salary} onChange={e => setFormData({...formData, salary: toEnglishDigits(e.target.value)})} placeholder="معاش پیشنهادی" className="w-full bg-gray-50 border rounded-2xl px-5 py-4 font-bold outline-none" required />
              ...
              <div className="relative">
                ...
                <input type="text" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} placeholder="آدرس دقیق" className="w-full bg-gray-50 border rounded-2xl px-11 py-4 font-bold outline-none" required />
              </div>
              ...
              <textarea rows={6} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="توضیحات و نیازمندی‌ها..." className="w-full bg-gray-50 border rounded-2xl px-5 py-4 font-bold outline-none resize-none"></textarea>
            </div>
          </form>
        </div>
