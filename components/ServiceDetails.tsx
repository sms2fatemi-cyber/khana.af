...
const ServiceDetails: React.FC<ServiceDetailsProps> = ({ service, onClose, onShowOnMap, isSaved, onToggleSave, t }) => {
  ...
  if (!service || !('providerName' in service)) return null;
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({ title: service.title, text: service.description, url: window.location.href }).catch(() => {});
    } else {
      alert("لینک کپی شد!");
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[5000] bg-white font-[Vazirmatn] flex flex-col h-[100dvh] w-full" dir="rtl">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar md:flex md:flex-row md:overflow-hidden">
        {/* Image Viewer */}
        <div 
            className="w-full h-[40vh] md:w-[60%] md:h-full bg-orange-950 relative shrink-0 flex items-center justify-center group select-none overflow-hidden"
        ...
        {/* Content Section */}
        <div className="bg-white md:flex-1 md:h-full md:overflow-y-auto p-6 md:p-8 space-y-6 rounded-t-[2rem] md:rounded-none -mt-6 md:mt-0 relative z-10 shadow-[0_-5px_20px_rgba(0,0,0,0.1)] md:shadow-none pb-4 no-scrollbar">
            <div className="max-w-2xl mx-auto md:mx-0">
                <div className="hidden md:flex justify-between items-start mb-6">
                    <h1 className="text-3xl font-black text-gray-900 leading-tight">{service.title}</h1>
                ...
                <h1 className="md:hidden text-2xl font-black text-gray-900 leading-tight mb-2 pt-2">{service.title}</h1>
                <div className="flex items-center gap-2 mb-6 text-gray-400 font-bold text-xs">
                   <Clock size={12} /> {getRelativeTime(service.date, 'dari')} در {service.city}
                </div>

                <div className="bg-orange-50/50 p-6 rounded-[2rem] flex justify-between items-center border border-orange-100 my-8">
                    <div>
                    ...
                    <button onClick={onShowOnMap} className="w-14 h-14 bg-white text-orange-600 rounded-2xl shadow-sm border border-orange-100 flex items-center justify-center active:scale-90 transition-transform"><MapPinned size={28} /></button>
                </div>

                <div className="space-y-6">
                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 flex items-center gap-3">
                       <div className="p-2 bg-white rounded-xl text-orange-600 shadow-sm"><User size={20} /></div>
                       <div>
                          <span className="block text-[10px] font-black text-gray-400">نام ارائه‌دهنده</span>
                          <span className="font-black text-sm text-gray-800">{service.providerName}</span>
                       </div>
                    </div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                        <h3 className="text-sm font-black text-gray-900 mb-2 flex items-center gap-2"><Wrench size={16} className="text-orange-600" /> دسته‌بندی</h3>
                        <p className="text-xs font-bold text-gray-600">{service.category}</p>
                    </div>
                </div>
            </div>
        </div>
      ...
      <div className="bg-white border-t p-4 flex gap-3 z-30 shrink-0 safe-area-bottom shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
        ...
        {service.showPhoneNumber !== false ? (
          !showContact ? (
            <button onClick={() => setShowContact(true)} className="flex-[2] bg-orange-700 text-white h-14 rounded-2xl font-black text-lg active:scale-95 shadow-xl transition-all">
                {t.contact_info}
            </button>
          ) : (
