...
import { User, Briefcase, Building2, Wrench, Plus, List, Map as MapIcon, Loader2, Languages, Search, ArrowRight } from 'lucide-react';
...
function App() {
  ...
  const performAutoCleanup = async () => {
    ...
    try {
      ...
      expiryThreshold.setDate(expiryThreshold.getDate() - 100);
      const expiryIso = expiryThreshold.toISOString();

      await Promise.all([
        supabase.from(TABLES.PROPERTIES).delete().lt('created_at', expiryIso),
        supabase.from(TABLES.JOBS).delete().lt('created_at', expiryIso),
        supabase.from(TABLES.SERVICES).delete().lt('created_at', expiryIso)
      ]);
    } catch (e) {
  ...
  const handleDeleteItem = async (item: Property | Job | Service) => {
    ...
    try {
      let tableName = properties.find(p => p.id === item.id) ? TABLES.PROPERTIES : jobs.find(j => j.id === item.id) ? TABLES.JOBS : TABLES.SERVICES;
  ...
  if (isAdminMode) {
    return <AdminPanel properties={properties} setProperties={setProperties} jobs={jobs} setJobs={setJobs} services={services} setServices={setServices} onExit={() => { setIsAdminMode(false); refreshData(); }} />;
  }
    ...
    <div className="flex flex-col h-screen bg-white font-[Vazirmatn] overflow-hidden" dir="rtl">
      <header className="h-[65px] bg-white border-b flex items-center justify-between px-4 z-[3000] shrink-0 shadow-sm">
        <div className="flex items-center gap-2">
          <button onClick={() => setViewMode(viewMode === 'list' ? 'map' : 'list')} className="bg-gray-900 text-white px-3 py-2 rounded-xl font-black text-[10px] md:hidden flex items-center gap-1">
            {viewMode === 'list' ? <><MapIcon size={14} /> <span>{t.map}</span></> : <><List size={14} /> <span>{t.list}</span></>}
          </button>
          <button onClick={toggleLanguage} className="flex items-center gap-1.5 px-3 py-2 bg-gray-50 border border-gray-100 rounded-xl text-[10px] font-black text-gray-600">
      ...
      <main className="flex-1 flex overflow-hidden relative">
            ...
            <div className="bg-white p-4 rounded-2xl border shadow-sm space-y-4">
              {filterCategory !== 'ALL' && (
                <div className="flex items-center justify-between bg-red-50 p-2 rounded-xl mb-2">
                   ...
                   <button onClick={() => setFilterCategory('ALL')} className="text-[9px] font-black text-gray-500 flex items-center gap-1">{lang === 'dari' ? 'پاک کردن' : 'پاکول'} <ArrowRight size={12} className="hidden" /></button>
                </div>
              )}
              <div className="flex gap-2">
                <div className="flex-1 relative flex items-center">
                  <input type="text" placeholder={appMode === 'ESTATE' ? t.search_estate : appMode === 'JOBS' ? t.search_jobs : t.search_services} className="w-full bg-gray-100 rounded-xl pr-4 pl-10 py-2.5 text-xs font-bold outline-none" value={displaySearch} onChange={(e) => setDisplaySearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearchClick()} />
                  <button onClick={handleSearchClick} className="absolute left-2 p-1.5 bg-[#a62626] text-white rounded-lg"><Search size={14} /></button>
                </div>
                <select value={selectedProvince} onChange={(e) => { setSelectedProvince(e.target.value); setDisplayLimit(20); }} className="bg-gray-100 rounded-xl px-2 py-2.5 text-[10px] font-black outline-none max-w-[100px]">
                  {t.provinces.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
              </div>
              {appMode === 'ESTATE' && (
                 <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
                    <button onClick={() => { setActiveDealFilter('ALL'); setDisplayLimit(20); }} className={`px-4 py-2 rounded-xl text-[10px] font-black whitespace-nowrap transition-all ${activeDealFilter === 'ALL' ? 'bg-[#a62626] text-white shadow-md' : 'bg-gray-100 text-gray-500'}`}>{t.all}</button>
                    <button onClick={() => { setActiveDealFilter(DealType.SALE); setDisplayLimit(20); }} className={`px-4 py-2 rounded-xl text-[10px] font-black whitespace-nowrap transition-all ${activeDealFilter === DealType.SALE ? 'bg-[#a62626] text-white shadow-md' : 'bg-gray-100 text-gray-500'}`}>{t.sale}</button>
                    <button onClick={() => { setActiveDealFilter(DealType.RENT); setDisplayLimit(20); }} className={`px-4 py-2 rounded-xl text-[10px] font-black whitespace-nowrap transition-all ${activeDealFilter === DealType.RENT ? 'bg-[#a62626] text-white shadow-md' : 'bg-gray-100 text-gray-500'}`}>{t.rent}</button>
                    <button onClick={() => { setActiveDealFilter(DealType.MORTGAGE); setDisplayLimit(20); }} className={`px-4 py-2 rounded-xl text-[10px] font-black whitespace-nowrap transition-all ${activeDealFilter === DealType.MORTGAGE ? 'bg-[#a62626] text-white shadow-md' : 'bg-gray-100 text-gray-500'}`}>{t.mortgage}</button>
                 </div>
              )}
            </div>
          </div>
          <div className="flex-1 overflow-y-auto px-3 pb-24 no-scrollbar space-y-3 pt-3">
            {isLoading ? <div className="flex justify-center py-24"><Loader2 className="animate-spin text-[#a62626]" size={40} /></div> : (
              <>
                {displayedItems.map(item => (
                  <div key={item.id}>
                    {appMode === 'ESTATE' ? <PropertyCard property={item as Property} onClick={() => handleSelectItem(item)} isVisited={visitedIds.has(item.id)} isSaved={savedIds.has(item.id)} onToggleSave={() => handleToggleSave(item)} onDelete={filterCategory === 'MY_ADS' ? () => handleDeleteItem(item) : undefined} onEdit={filterCategory === 'MY_ADS' ? () => handleEditItem(item) : undefined} lang={lang} /> :
                     appMode === 'JOBS' ? <JobCard job={item as Job} onClick={() => handleSelectItem(item)} isVisited={visitedIds.has(item.id)} isSaved={savedIds.has(item.id)} onToggleSave={() => handleToggleSave(item)} onDelete={filterCategory === 'MY_ADS' ? () => handleDeleteItem(item) : undefined} onEdit={filterCategory === 'MY_ADS' ? () => handleEditItem(item) : undefined} lang={lang} /> :
                     <ServiceCard service={item as Service} onClick={() => handleSelectItem(item)} isVisited={visitedIds.has(item.id)} isSaved={savedIds.has(item.id)} onToggleSave={() => handleToggleSave(item)} onDelete={filterCategory === 'MY_ADS' ? () => handleDeleteItem(item) : undefined} onEdit={filterCategory === 'MY_ADS' ? () => handleEditItem(item) : undefined} lang={lang} />
                    }
                  </div>
                ))}
                {filteredItems.length > displayLimit && <button onClick={() => setDisplayLimit(prev => prev + 20)} className="w-full py-4 bg-gray-50 border rounded-2xl text-[11px] font-black text-gray-500">مشاهده بیشتر</button>}
        ...
        <div className="fixed bottom-0 left-0 right-0 h-[70px] bg-white/95 backdrop-blur-md border-t flex items-center justify-around z-[4000] px-2 pb-env">
          ...
          <button onClick={handleOpenAddModal} className="w-14 h-14 bg-[#a62626] text-white rounded-2xl flex items-center justify-center shadow-xl -top-6 relative active:scale-90 border-4 border-white"><Plus size={32} /></button>
